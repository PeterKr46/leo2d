...
public class CircleCollider extends Collider {
    ...
    public Physics.RaycastHit cast(Ray ray) {
        Ray orth = new Ray(transform.getPosition().add(offset), ray.getDirection().getOrth());
    ...
    public Rect getBounds() {
        Vector min = transform.getPosition().add(offset).subtract(radius, radius);
    ...
    public void update() {
        if(Camera.main().debug()) {
            ...
            volty.circle(transform.getPosition().add(offset), radius, 0.5, new double[]{0.2, 1, 0.2, 0.6});
            getBounds().visualize();
        }
    }
